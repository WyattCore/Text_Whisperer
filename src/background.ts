...
function set_popup_text(text:string){
    pop_text = text;
}
...
chrome.runtime.onInstalled.addListener(() => {   //adds the extension to the right click menu
...
function set_chat_response(chat_resp: string){
...
async function give_info(selected_text: string, src_url:any){
    console.log("give_info function");
    try{
        const data = await http(
            back_end_port+'/selected_text',
            back_end_port+'/chat_output', 
            selected_text,
            src_url
        );
        if(data.response){
